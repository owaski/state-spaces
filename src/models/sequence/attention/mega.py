...
class MegaLayer(nn.Module):
    def __init__(
        ...
        assert dropout == 0. and transposed == False
    ...
    def forward(self, x, residual = None, state=None):
        residual = default(residual, x)

        ema_output = x if self.disable_ema else self.multi_headed_ema(x)
        ...
        return update_gate * H + (1 - update_gate) * residual, None
